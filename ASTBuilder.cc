...
ASTNode* ASTBuilder::GetASTRoot() const {
  if (ast_.size() != 1) return nullptr;

  return ast_.top().get();
}

void ASTBuilder::_Reset() {
  while (!ast_.empty()) ast_.pop();
}
...
bool ASTBuilder::Build(const char* regex, std::size_t len) {
  ...
  for (std::size_t i = 0; i < len; ++i) {
    ...
    switch (ch) {
      case kLeft:
        if (!prevHasSign) {
          if (!_AddSymbol(symbols, kConcat)) return false;
      ...
      case kRight:
        while (symbols.top() != kLeft) {
          if (!_ProcessNode(symbols.top())) return false;
        ...
        symbols
            .pop();  // pop '(', if () is empty, prevHasSign should not change
        break;
      ...
      case kKleene:
      case kAlter:
        if (!_AddSymbol(symbols, ch)) return false;

        prevHasSign = (ch != kKleene && ch != kPlus &&
                       ch != kQuestion);  // for example, "a*b"
        break;
      ...
      default:
        ...
        ast_.push(std::unique_ptr<CharNode>(
            new CharNode(ch == kDot ? ASTNode::kAny : ch)));
        prevHasSign = false;
        break;
    }
  }
...
std::unique_ptr<RangeNode> ASTBuilder::_ProcessRange(const char* regex,
                                                     std::size_t len,
                                                     std::size_t& offset) {
...
ASTBuilder::PriorityLevel ASTBuilder::_Priority(int ch) const {
  switch (ch) {
    ...
    case kQuestion:
    case kKleene:
      return PriorityLevel::LEVELTHREE;
    case kConcat:
      return PriorityLevel::LEVELTWO;
    case kAlter:
      return PriorityLevel::LEVELONE;

    default:
      return PriorityLevel::LEVELZERO;
  }
}
...
bool ASTBuilder::_AddSymbol(std::stack<int>& symbols, int newSymbol) {
  while (!symbols.empty() && _Priority(newSymbol) <= _Priority(symbols.top())) {
    ...
    if (!_ProcessNode(sym)) return false;
  }
